...
def run_immunogenicity_analysis(input_file, download_dir, threshold, callback=None):
    try:
        subprocess.run(["python", "vaccine_analysis/analyze_immunogenicity.py", input_file, download_dir, str(threshold)], check=True)
        print("Immunogenicity analysis completed successfully.")
        if callback:
...
class VaccineAnalysisApp:
    def __init__(self, root):
        ...
        self.threshold_label = tk.Label(root, text="Enter maximum combined score threshold:")
    ...
    def run_analysis(self):
        ...
        if not sequences or not linker or not threshold or not output_dir:
            messagebox.showerror("Error", "Please fill in all fields.")
            return
